CREATE TABLE [dbo].[GenreHierarchy] (
    ...
    [CreatedUtc]    DATETIME       CONSTRAINT [DF_GenreHierarchy_CreatedUtc] DEFAULT (getutcdate()) NOT NULL,
    [CreatedBy]     NVARCHAR (255) CONSTRAINT [DF_GenreHierarchy_CreatedBy] DEFAULT (suser_sname()) NOT NULL,
    [ModifiedUtc]   DATETIME       NULL,
    [ModifiedBy]    NVARCHAR (255) NULL,
    CONSTRAINT [PK_GenreHierarchy] PRIMARY KEY NONCLUSTERED ([counter] ASC),
    CONSTRAINT [FK_GenreHierarchy_GenreId] FOREIGN KEY ([GenreId]) REFERENCES [dbo].[Genre] ([GenreId]),
    CONSTRAINT [FK_GenreHierarchy_ParentGenreId] FOREIGN KEY ([ParentGenreId]) REFERENCES [dbo].[Genre] ([GenreId])
);
